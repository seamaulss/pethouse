...
<div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800 flex items-center gap-3">
            <i class="fas fa-edit text-teal"></i>
            Edit Kegiatan
        </h1>
        <p class="text-gray-600 mt-2">
            Perbarui informasi kegiatan layanan
        </p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.master-kegiatan.update', $kegiatan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi"
                          id="deskripsi"
                          rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-teal focus:border-teal">{{ old('deskripsi', $kegiatan->deskripsi) }}</textarea>
            </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if($kegiatan->icon)
                            <span class="text-sm text-gray-500 whitespace-nowrap">
                                <i class="fas fa-{{ $kegiatan->icon }}"></i> {{ $kegiatan->icon }}
                            </span>
                        @endif
                    </div>
                ...
                <div>
                    ...
                    <select name="warna"
                        ...
                        @foreach($warnaList as $warna)
                            <option value="{{ $warna }}" {{ old('warna', $kegiatan->warna) == $warna ? 'selected' : '' }}>
                                {{ ucfirst($warna) }}
                            </option>
                        @endforeach
                    </select>
                    @if($kegiatan->warna)
                        <div class="mt-2">
                            <span class="px-3 py-1 rounded-full text-xs bg-{{ $kegiatan->warna }}-100 text-{{ $kegiatan->warna }}-800">
                                {{ ucfirst($kegiatan->warna) }}
                            </span>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="flex items-center gap-6 mt-2">
                        <label class="inline-flex items-center">
                            <input type="radio"
                                   name="aktif"
                                   value="ya"
                                   {{ old('aktif', $kegiatan->aktif) == 'ya' ? 'checked' : '' }}
                        ...
                        <label class="inline-flex items-center">
                            <input type="radio"
                                   name="aktif"
                                   value="tidak"
                                   {{ old('aktif', $kegiatan->aktif) == 'tidak' ? 'checked' : '' }}
